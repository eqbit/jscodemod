...
(Note: I actually couldn't get even a longer form of this prompt to work as a codemod, but was able to get the AI to [generate a codemod](https://beta.openai.com/playground/p/fHLPwP4nrF5emnrtJv921rOZ?model=code-gpt-3.5-turbo-instruct) that would handle some cases.)
...
And it'll return a plugin that looks like it basically does the right thing ([playground link](https://beta.openai.com/playground/p/aIiWGsn3gdp3OEdab1QTD9Lg?model=code-gpt-3.5-turbo-instruct)). You can then use that with [this codemod runner](../README.md#babel-plugin):
